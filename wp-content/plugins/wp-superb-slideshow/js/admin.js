...
//add replace on fileObj.filePath.replace("/wordpress/wp-admin/","") to workaround on windows server
	...
	test=file;
	var params = 'myajax=1&filename='+ superb_upload_dir+"/"+file.name + '&task=superb_resize_image_and_add&album_id='+jQuery('#multi_album_id').val();
...
function on_queue_completed(file)
{
	...
	superb_refresh_images_table(superb_current_album_id);
...
function superb_refresh_albums_table()
{
	...
	//sleep(2000);
	jQuery.post('index.php', 'task=superb_get_albums_table', function(res)
	{
		superb_set_data_tables();
		if (res.status == 'ok') 
		{
...
function superb_refresh_images_table(album_id)
{
	...
	jQuery.post('index.php', 'task=superb_get_albums_images_table&album_id='+album_id, function(res)
	{
		superb_set_data_tables();
		if (res.status == 'ok') 
		{
...
function re_order()
{
	...
	if( task == 'superb_reorder_album' )
		jQuery('#albums_table tbody').html('<tr><td colspan="7" align="center"><img src="'+loading.src+'" alt="" /><br/>Please wait, refreshing albums</td></tr>');
	if( task == 'superb_reorder_image' )
	...
	jQuery.get('index.php?' + params, function(res)
	{
		if( task == 'superb_reorder_image' )
		{
			superb_refresh_images_table(album_id);
		}
		if( task == 'superb_reorder_album' )
			superb_refresh_albums_table();
...
var tpager = null;
var ipager = null;
function superb_set_data_tables()
{
	//add event to bulk actions
	jQuery('.bulk_form').live('submit', superb_submit_bulk);
...
function superb_submit_bulk()
{
	var task = this.task.value;
	var album_id = null;
	if( task == 'superb_bulk_delete_albums' || task == 'superb_bulk_enable_albums' || task == 'superb_bulk_disable_albums')
	{
	...
	else if( task == 'superb_bulk_delete_images' || task == 'superb_bulk_enable_images' || task == 'superb_bulk_disable_images' )
	{
...
{
	//preload images
	loading.src = superb_url + '/images/loading.gif';
...
/*
		var ops = {
					'uploader'		: superb_url + '/js/uploadify-2.1.4/uploadify.swf',
					'script'		: superb_url + '/js/uploadify-2.1.4/uploadify.php',
					'cancelImg' 	: superb_url + '/js/uploadify-2.1.4/cancel.png',
				    'folder'    	: superb_upload_dir,
				    'auto'			: true,
					'multi'			: true,
					'fileExt'        : '*.jpg;*.gif;*.png',
...
*/
		var swfu;
			...
			var settings = {
				flash_url : superb_url+"/js/swfupload/js/swfupload.swf",
				upload_url: superb_url+"/js/swfupload/js/upload.php",
				post_params: {"PHPSESSID" : "<?php echo session_id(); ?>","folder":superb_upload_dir, "secw":superb_secw, nopar: "n" + new Date().getTime()},
				...
				// Button settings
				button_image_url: superb_url+"/js/swfupload/js/TestImageNoText_65x29.png",
				button_width: "130",
				button_height: "29",
	...
	jQuery('.superb_delete_album').live('click', superb_delete_album);
	...
	jQuery('table#album_images .enable, table#albums_table .enable').live('click', superb_enable);
	jQuery('table#album_images .disable, table#albums_table .disable').live('click', superb_disable);
